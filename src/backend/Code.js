...
function doGet(e) {
  try {
    // Attempt to load with path prefix first (due to clasp structure)
    let template;
    try {
      template = HtmlService.createTemplateFromFile("frontend/Dashboard");
    } catch (e) {
      // Fallback if flattened or named differently
    ...
    return HtmlService.createHtmlOutput(
      '<h1>System Error</h1><p>Failed to load application. Please contact support.</p><p style="color:gray;font-size:small">Error: ' +
        error.message +
        "</p>"
    );
  }
